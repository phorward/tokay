...
impl ImlInstance {
    pub fn derive(&self, program: &mut ImlProgram, current: (&ImlRefParselet, usize)) -> ImlValue {
        let mut instance = self.clone(); // temporary, as it is modified.

        match &*instance.target {
            ImlValue::Shared(shared) => {
                let shared = shared.borrow();
                if let ImlValue::Instance(instance) = &*shared {
                    instance.derive(program, current)
                }
                else {
                    shared.clone()
                }
            ...
            ImlValue::Parselet(parselet) => {
                ...
                for (name, default) in parselet.generics.iter() {
                    ...
                    generics.insert(
                        name.clone(),
                        if let Some(ImlValue::Instance(instance)) = arg.1 {
                            Some(instance.derive(program, current))
                        } else {
                            arg.1
                        },
                    );
            ...
            unimpl => unimplemented!("{:?} is not implemented yet", unimpl),
        }
    }
}
